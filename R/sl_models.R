...
sl_example_model = function(
    model_dir = file.path(tempdir(), "sleeper-example-models"),
    overwrite = FALSE
) {

  files = paste0("fold2_", c("binary", "nonwear"), "_balanced_RF.sav")
  files = system.file("extdata", files, package = "sleeper")
  dir.create(model_dir, recursive = TRUE, showWarnings = FALSE)
  file.copy(files, file.path(model_dir, basename(files)), overwrite = overwrite)

  files = model_df(model_dir = model_dir, folds = 2)
  files
}
