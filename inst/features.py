...
def compute_features(data, time_interval):
  _, ENMO_stats, angle_z_stats, LIDS_stats = compute_features_out(data, time_interval)
  feat = np.hstack((ENMO_stats, angle_z_stats, LIDS_stats))

  return feat

def get_resampled_time(data, time_interval):
  ...
  timestamp_agg, _ = get_stats(df['timestamp'], z, time_interval)

  return timestamp_agg

def compute_features_out(data, time_interval):
  df = pd.DataFrame(data, columns=['timestamp','x','y','z'])
  df['timestamp'] = pd.to_datetime(df['timestamp'], unit='s')
  ...
  return timestamp_agg, ENMO_stats, angle_z_stats, LIDS_stats
